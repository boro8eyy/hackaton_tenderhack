<script lang="ts">
	...
	async function handleLogin() {
		...
		try {
			const response = (await login({ login: loginValue, password })) as {
				access_token: string
			}
			...
			if (e instanceof ApiError) {
				if (e.status === 401) {
					...
					errorMessage = e.data?.detail || 'Ошибка входа'
				}
			} else {
	...
	function handleElectronicSignature() {
		// TODO: Implement electronic signature login
		console.log('Electronic signature login')
	}
...
<main class="login-page">
	<div class="login-container">
		...
		<div class="register-link">
			...
			<button type="button" class="register-button" onclick={goToRegister}>
				Зарегистрируйтесь
			</button>
		</div>
	</div>
