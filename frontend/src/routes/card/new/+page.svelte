<script lang="ts">
	import { createCard, type CardCreateRequest } from '$lib/api/card'
	import { getSteList, type Ste } from '$lib/api/ste'
	...
	// Form state
	let name = $state('')
	let selectedSteIds = $state<Set<number>>(new Set())
	...
	// Available STEs (only those not already in a card)
	let availableStes = $state<Ste[]>([])
	let stesLoading = $state(true)

	// Filtered STEs by search
	...
	onMount(async () => {
		await loadAvailableStes()
	})

	async function loadAvailableStes() {
		stesLoading = true
		try {
			const allStes = await getSteList()
			// Фильтруем только СТЕ без привязки к карточке
			availableStes = allStes.filter(ste => !ste.card_id)
		} catch (e) {
	...
	function toggleSte(steId: number) {
		const newSet = new Set(selectedSteIds)
		if (newSet.has(steId)) {
			newSet.delete(steId)
		} else {
			newSet.add(steId)
		}
		selectedSteIds = newSet
	}
	...
	async function handleSubmit() {
		...
		try {
			const data: CardCreateRequest = {
				...
				ste_ids: Array.from(selectedSteIds),
			}
			await createCard(data)
			window.location.href = '/card'
		} catch (e) {
			console.error('Failed to create card:', e)
			error = e instanceof Error ? e.message : 'Ошибка при создании карточки'
		} finally {
			saving = false
		}
	}
...
<svelte:head>
	<title>Добавить карточку | TenderHack</title>
</svelte:head>
			...
			>
				...
				<div class="form-section">
					<div class="section-header">
						<span class="form-label" id="ste-selection-label"
							>Выберите СТЕ для карточки</span
						>
						<div class="section-actions">
							<button type="button" class="btn-text" onclick={selectAllVisible}>
								Выбрать все
							</button>
					...
					{#if selectedSteIds.size > 0}
						<div class="selected-count">
							Выбрано: {selectedSteIds.size} СТЕ
						</div>
					{/if}
					...
					<div class="ste-list">
						...
						{:else if filteredStes.length === 0}
							<div class="empty-state">
								{steSearchQuery
									? 'СТЕ не найдены'
									: 'Нет доступных СТЕ без карточки'}
							</div>
						{:else}
							{#each filteredStes as ste}
								<label
									class="ste-item"
									class:selected={selectedSteIds.has(ste.id)}
								>
									<Checkbox
										checked={selectedSteIds.has(ste.id)}
